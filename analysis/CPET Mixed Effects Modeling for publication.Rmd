---
title: "CPET Mixed regression models"
...
## Read in the csv of clean data
```{r Read in data, message = FALSE, include = FALSE}
all_GK_raw  <-  read_delim("/Users/nickc/Documents/Academics/Thesis Research/CPET/UCI/cleaned data/all_data.csv","," )
...
# write_csv(all_GK_bybout, file = "C:/Users/nickc/Documents/Academics/Thesis Research/CPET/UCI/cleaned data/all_data2.csv")
```

## Convert bout to factor.
...
## Pull out the avgHR at each "on-transient" bouts 1-5.
```{r}
...
##Plot the average HR data distributions.
```{r}
...
names(dat.HRon)[1:2] <- c("bout","avgHR")

dat.HRon$bout <- as.factor(dat.HRon$bout)

# Facet_grid allows us to see a different dot plot for each group's summary stats
ggplot(dat.HRon, aes(bout, avgHR)) + geom_point(aes(shape = Tanner))+
  geom_errorbar(
